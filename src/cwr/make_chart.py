...
def command_line_parser():
    ...
    parser = argparse.ArgumentParser(description="Extract chart of accounts")
    parser.add_argument(
        "--chart",
        default="chart.csv",
        metavar="CHART",
    ...
    parser.add_argument(
        ...
        metavar="BALANCE",
        help=(
            "Balance statement dumped from Church Windows as a CSV file."
...
def scan_chart(chart):
    ...
    with open(chart, encoding="utf-8") as csvfile:
...
def scan_balance(balance):
    ...
    with open(balance, encoding="utf-8") as csvfile:
        for line in csv.reader(csvfile):
