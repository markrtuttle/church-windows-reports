...
def select_by_date(entries, date_start, date_end, posted_start=None, posted_end=None):
    return [
        entry
        ...
        or (
            posted_start
            and posted_end
            and entry.posted_is(posted_start, posted_end)
            and entry.date_is(None, posted_end)
        )
    ]
...
def select_by_amount(entries, low=None, high=None):
    ...
    def select(amount):
        if amount is None:
            return None
        amt = abs(amount)
        return (low is None or low <= amt) and (high is None or amt <= high)

    return [entry for entry in entries if select(entry.credit() or entry.debit())]


def select_debit(entries):
    return [entry for entry in entries if accountt.is_debit_number(entry.number())]
